...
def get_relaunch_hits(tasks, args):
    ...
    assert len(one_assignment_tasks) + len(two_assignment_tasks) != 0, 'Stage 4 initial launch done'
...
def prepare_launch(args):
    assert utils.all_rounds_are_done(args), 'You must complete all rounds of stages 1, 2, and 3 first'

    tasks = utils.get_stage_4_tasks(args)
