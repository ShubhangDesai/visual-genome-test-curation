...
def prepare_launch(args):
    ...
    knowledge, tasks = utils.get_knowledge_file(args), utils.get_stage_4_tasks(args)
    ...
    for task in tasks:
        ...
        assert 'worker_1' in task_knowledge and 'worker_2' in task_knowledge, 'You must finish stage 4 initial launches first'

        if 'final_answer' not in task_knowledge:
            remaining_tasks.append(task)

    assert len(remaining_tasks) != 0, 'Stage 4 disagreement launch done'
